...
bool CGZip::Close()
{
	...
	int result = gzclose((gzFile)m_gzf);
	m_gzf=0;
...
bool CGZip::WriteBuffer( void* pBuffer, size_t nBytes)
{
	if (!IsOpen() || !IsWriting())
		return false;
	int written=gzwrite((gzFile)m_gzf, pBuffer, nBytes);
...
bool CGZip::Flush( EFlushMode eFlush)
{
	...
	return gzflush((gzFile)m_gzf, eFlush)==Z_OK;
}
...
int CGZip::ReadBuffer( voidp* ppBuffer, size_t& nBytes)
{
	...
	while( read>0)
	{
		...
		read=gzread((gzFile)m_gzf, pBuffer, m_bufferSize);
		if (read>0)
		{
...
int CGZip::ReadBufferSize( voidp pBuffer, size_t nBytes)
{
	...
	return gzread((gzFile)m_gzf, pBuffer, nBytes);
}
...
void CGZip::UpdateParams()
{
	...
	gzsetparams((gzFile)m_gzf, m_eCompressionMode, m_eStrategy);
};
